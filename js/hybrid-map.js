...
function InitializePage(error, results) {
    ...
    //
    statesSelect.style('width', vs.statesSelect.w + 'px');
    //
    ResizePage();
    //
...
function UpdateInfo() {
    ...
    infoImageGs = infoImageGs.enter().append('g').classed('info-image-g', true).each(function (datum) {
        d3.select(this).append('image').attr('x', 0).attr('y', 0).attr('width', vs.info.w).attr('height', vs.info.w * 432 / 396).attr('xlink:href', function () {
    ...
    infoTextGs = infoTextGs.enter().append('g').classed('info-text-g', true).attr('transform', 'translate(' + vs.info.w / 2 + ',' + vs.info.w * 432 / 396 + ')').each(function (datum) {
        d3.select(this).append('text').attr('x', 0).attr('y', function (d, i) {
            return (i + 1) * 15;
        }).text(function () {
            return datum.id;
        });
