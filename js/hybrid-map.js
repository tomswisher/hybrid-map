...
function HybridMapClass() {
    ...
    that.width = function (_) {
        console.log(''.padStart(2 * stackLevel) + "%cthat.width = _ => {", "color:blue");
        return _ !== undefined ? (_width = _, that) : _width;
    };
    var _height = 0;
    that.height = function (_) {
        console.log(''.padStart(2 * stackLevel) + "%cthat.height = _ => {", "color:blue");
        return _ !== undefined ? (_height = _, that) : _height;
    };
    var _statesFeatures = null;
    that.statesFeatures = function (_) {
        console.log(''.padStart(2 * stackLevel) + "%cthat.statesFeatures = _ => {", "color:blue");
        return _ !== undefined ? (_statesFeatures = _, that) : _statesFeatures;
    };
    var _vertices = null;
    ...
    that.UpdateFilters = function () {
        ...
        filtersDiv.style('width', vs.filters.w + 'px').style('height', vs.filters.h + 'px').style('left', '0px').style('top', vs.states.h + 'px');
    ...
    that.UpdateOptions = function () {
        ...
        optionsDiv.style('left', '0px').style('top', Math.max(vs.svg.h, vs.states.h + vs.filters.h) + 'px');
...
function UpdatePageDimensions() {
    ...
    hybridMapObj.width(vs.states.w).height(vs.states.h).UpdateStates().UpdateInfo().UpdateFilters().UpdateVerticesEdges().UpdateSimulation().UpdateOptions();
    TestApp('UpdatePageDimensions', -1);
}
