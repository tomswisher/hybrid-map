...
function InitializePage() {
	...
	mapInstance.csvData(csvDataURL, function() {
        mapInstance.jsonData(jsonDataURL, function() {
            ResizePage();
...
function UpdateInfobox(source) {
	...
	categoryRowGs
		...
		.on('mouseover', function(d) {
            ...
            UpdateFilters('categoryRowGs');
			UpdateHover('categoryRowGs');
			UpdateInfobox('categoryRowGs');
		})
		.each(function(categoryName) {
            var categoryLabelWidth = rowWidth - 4*vs.categoryMarginX - vs.categoryGradeWidth;
            // console.log('categoryLabelWidth', categoryLabelWidth);
            //
...
function MapObject() {
	...
	this.csvData = function(csvDataURL, callback) {
		...
		d3.csv(csvDataURL, function(csvData) {
            ...
            // that._categoryNames.unshift('Overall Grade');
			// UpdateInfobox('csvData');
			// that.UpdateMap();
			return callback.call(that);
		});
	};

	this.jsonData = function(jsonDataURL, callback) {
		var that = this;
	...
	this.UpdateMap = function() {
		...
		statePaths
			...
			.on('mousedown', function(d) {
                ...
                hoverText2 = mapInstance._category;
				UpdateHover();
				UpdateInfobox('statePaths mousedown');
				mapInstance.UpdateMap();
			})
			.each(function(d) {
				if (d.properties.name === 'DC') {
		...
		statePaths
			.attr('transform', 'scale(1)')
			.each(function(d) {
				var svgBoundingRect = mapSVG.node().getBoundingClientRect();
				var stateBoundingRect = d3.select(this).node().getBoundingClientRect();
				...
				if (stateHovered === d.properties.name || stateClicked === d.properties.name) {
					...
					mapSVG.selectAll('g.hover-g').node().parentNode.appendChild(mapSVG.selectAll('g.hover-g').node());
				}
			});
		//---
		stateGs
	...
	hoverG
		.attr('transform', function() {
			...
			if (stateClicked !== undefined) {
				...
				mouseX = d3.mouse(mapSVG.node())[0];
				mouseY = d3.mouse(mapSVG.node())[1];
